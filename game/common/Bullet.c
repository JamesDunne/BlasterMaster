...
void Bullet_Touched(entity e, entity o) {
	...
	if (o == *(host->world)) {
		...
		if (self->flags & FLAG_BOUNCE) {
			if (self->collide_flags & COLLIDE_BELOW) self->dy = -(ABS(self->cdy) * 0.5);
			if (self->collide_flags & COLLIDE_LEFT) self->dx = -self->cdx;
			if (self->collide_flags & COLLIDE_RIGHT) self->dx = -self->cdx;
		}
		return;
	}
...
void Bullet_Draw(entity e) {
	...
	switch (self->kind) {
		case BULLET_SMALL:		host->put_sprite(nx, ny, 0, 0x52); break;
		case BULLET_MEDIUM:		host->put_sprite(nx, ny, 1, 0x45); break;
		case BULLET_MEDIUM_RED:	host->put_sprite(nx, ny, 0, 0x45); break;
		case BULLET_LARGE:		host->put_sprite(nx, ny, 1, 0xD6); break;
		case BULLET_CANNON:
			switch (self->face_dir) {
				case 0: host->put_sprite(nx, ny, 1, 0x65); break;
				case 2: host->put_sprite_hflip(nx, ny, 1, 0x55); break;
				case 4: host->put_sprite_vflip(nx, ny, 1, 0x65); break;
				case 6: host->put_sprite(nx, ny, 1, 0x55); break;
			}
			break;
		case BULLET_HYPER:
			switch (self->face_dir) {
				case 0:
					host->put_sprite(nx, ny	 , 1, 0x84);
					host->put_sprite(nx, ny + 16, 1, 0x94);
					host->put_sprite(nx, ny + 32, 1, 0xA4);
					break;
				case 2:
					host->put_sprite_hflip(nx	 , ny, 1, 0xB4);
					host->put_sprite_hflip(nx - 16, ny, 1, 0xC4);
					host->put_sprite_hflip(nx - 32, ny, 1, 0xD4);
					break;
				case 4:
					host->put_sprite_vflip(nx, ny	 , 1, 0x84);
					host->put_sprite_vflip(nx, ny - 16, 1, 0x94);
					host->put_sprite_vflip(nx, ny - 32, 1, 0xA4);
					break;
				case 6:
					host->put_sprite(nx	 , ny, 1, 0xB4);
					host->put_sprite(nx + 16, ny, 1, 0xC4);
					host->put_sprite(nx + 32, ny, 1, 0xD4);
					break;
			}
			break;
		case BULLET_CRUSHER:
		...
		case BULLET_HOMING_MISSILE:
			switch (self->face_dir) {
				case 0: host->put_sprite(nx, ny, 1, 0xF7); break;
				case 1: host->put_sprite_hflip(nx, ny, 1, 0xE7); break;
				case 2: host->put_sprite_hflip(nx, ny, 1, 0xF6); break;
				case 3: host->put_sprite_hvflip(nx, ny, 1, 0xE7); break;
				case 4: host->put_sprite_vflip(nx, ny, 1, 0xF7); break;
				case 5: host->put_sprite_vflip(nx, ny, 1, 0xE7); break;
				case 6: host->put_sprite(nx, ny, 1, 0xF6); break;
				case 7: host->put_sprite(nx, ny, 1, 0xE7); break;
			};
			break;
	}
};
