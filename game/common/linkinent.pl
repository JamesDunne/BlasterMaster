...
open (CLASSES_H, "<classes.h") || die "Couldn't open classes.h\n";
open (OUT_CLASSES_H, ">classes.h2") || die "Couldn't open classes.h2 for writing\n";
...
while (<CLASSES_H>) {
	...
	if (substr($line, 0, 74) eq "///---Do not change this line!! It is used by scripts to add entities!!---") {
		...
		if (!$wrote_class) {
			...
			print OUT_CLASSES_H "#include \"$entname.h\"\n";
		}
	}
}
...
system ("gcc -MM -I../../common $entname.c >> Makefile.2");

# Now, overwrite the old files with the new ones:
unlink "classes.h";
unlink "interface.c";
unlink "Makefile";
rename "classes.h2", "classes.h";
