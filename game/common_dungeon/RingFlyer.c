	...
	if (self->face_dir == 6) {
		switch (self->frame) {
			case 0:
				...
				host->put_sprite      (nx + 16, ny     , 1, 0x6D);
				host->put_sprite_vflip(nx     , ny + 16, 1, 0x6C);
				host->put_sprite_vflip(nx + 16, ny + 16, 1, 0x6D);
				break;
			case 1:
			case 3:
				host->put_sprite(nx     , ny     , 1, 0x7C);
				host->put_sprite(nx + 16, ny     , 1, 0x7D);
				host->put_sprite(nx     , ny + 16, 1, 0x8C);
				host->put_sprite(nx + 16, ny + 16, 1, 0x8D);
				break;
			case 2:
				host->put_sprite(nx     , ny +  8, 1, 0x8E);
				host->put_sprite(nx + 16, ny +  8, 1, 0x8F);
				break;
		}
	} else if (self->face_dir == 2) {
		switch (self->frame) {
			case 0:
				host->put_sprite_hflip (nx     , ny     , 1, 0x6D);
				host->put_sprite_hflip (nx + 16, ny     , 1, 0x6C);
				host->put_sprite_hvflip(nx     , ny + 16, 1, 0x6D);
				host->put_sprite_hvflip(nx + 16, ny + 16, 1, 0x6C);
				break;
			case 1:
			case 3:
				host->put_sprite_hflip(nx     , ny     , 1, 0x7D);
				host->put_sprite_hflip(nx + 16, ny     , 1, 0x7C);
				host->put_sprite_hflip(nx     , ny + 16, 1, 0x8D);
				host->put_sprite_hflip(nx + 16, ny + 16, 1, 0x8C);
				break;
			case 2:
				host->put_sprite_hflip(nx     , ny +  8, 1, 0x8F);
				host->put_sprite_hflip(nx + 16, ny +  8, 1, 0x8E);
				break;
		}
	}
};
...
void RingFlyer_Preview(int nx, int ny) {
	...
	host->put_sprite_vflip(nx + 8, ny + 8, 1, 0x6D);
}
