...
void post_render() {
	...
	xofs = fmod(*(host->old_screen_mx), 32.0);
	yofs = fmod(*(host->old_screen_my), 32.0);

	// Draw overlaid portions (MAPFLAG_JASON flag)
	for (y = 0; y <= (*(host->screen_h) / 32.0); ++y) {
	...
	if (tank != NULL) {
		// Put the GUN meter in:
		for (i = 3; i >= 0; --i)
			if (tank->ammo.guns >= (i << 6) + 63)
				host->put_sprite(32, 96 + ((3-i)<<4), 5, 0xB7);
			else if (tank->ammo.guns >= (i << 6) + 31)
				host->put_sprite(32, 96 + ((3-i)<<4), 5, 0xD7);
			else
				host->put_sprite(32, 96 + ((3-i)<<4), 5, 0xF7);
	...
	for (i = 3; i >= 0; --i)
		if ((*player)->health >= (i << 6) + 63)
			host->put_sprite(32, 304 + ((3-i)<<4), 4, 0x67);
		else if ((*player)->health >= (i << 6) + 31)
			host->put_sprite(32, 304 + ((3-i)<<4), 4, 0x87);
		else
			host->put_sprite(32, 304 + ((3-i)<<4), 4, 0xA7);
	host->put_sprite(32, 368, 4, 0xE5);
	host->put_sprite(32, 384, 4, 0xF5);
}
...
///---Do not change this line!! It is used by scripts to add new entities!!---
	DEFINE_CLASS(CLASS_RINGFLYER, ClassProperties_RingFlyer, SPAWNFLAG_MAPSPAWNABLE)
