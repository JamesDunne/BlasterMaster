...
void wMain_Click(TWindow *self, int mX, int mY, int btn) {
	...
	if ((mX <= 6) || (mY <= 27) || (mX > 512+6) || (mY > 512+27)) return;
...
void wMain_KeyDown(SDL_KeyboardEvent *kev) {
	int		i;
	int		x, y, t;
	int		maxlen, len;
	switch ( kev->keysym.sym ) {
		...
		case SDLK_c:	// CTRL+C copies the selection box:
			if ( (kev->keysym.mod & KMOD_CTRL) && (operation == OPERATION_SELECTEDBOX) ) {
			...
			if ( (kev->keysym.mod & KMOD_CTRL) ) {
				if (clipboard) {
					...
					for (y=0; y<clipboard_h; ++y) {
						...
						for (x=0; x<clipboard_w; ++x) {
							if (x+mx+screen_mx >= map.width) break;
...
void wMain_KeyUp(SDL_KeyboardEvent *kev) {
	int		i;
	int		x, y, t;
	int		maxlen, len;
...
void gameloop() {
	...
	TWindow_Init(wMain, 0, 0, 512+12, 512+27+26, "Map Editor");
	...
	TWindow_Init(wTiles, 526, 0, 256+12, 256+48+27+26, "Tile Chooser");
	...
	TWindow_Init(wEnts, 526, 357, 256+12, 34+27+26, "Entities");
	...
	TWindow_Init(wDoors, 526, 446, 256+12, 64+27+26, "Doors");
	...
	TWindow_Init(wEntSelect, 384, 24, 64+8+12+12, 64+27, "Choose entity...");
...
*/
			...
			if ( event.type == SDL_KEYDOWN ) {
				...
				switch ( event.key.keysym.sym ) {
					...
					case SDLK_DOWN:		dy =  1; break;
					case SDLK_c:	// CTRL+C copies the selection box:
						if ( (event.key.keysym.mod & KMOD_CTRL) && (operation == OPERATION_SELECTEDBOX) ) {
							// Fill clipboard with the data:
							clipboard_w = (box_x2 - box_x1) + 1;
							clipboard_h = (box_y2 - box_y1) + 1;
							if (clipboard) free(clipboard);
		...
		if ((operation == OPERATION_SELECTBOX) || (operation == OPERATION_SELECTEDBOX)) {
			...
			glPushMatrix();
				glBindTexture(GL_TEXTURE_2D, 0);
				glTranslatef(wMain->X + 6, wMain->Y + 27, 0.1);
				glColor4f(0.0, 0.0, 0.4, 0.25);
				glBegin(GL_QUADS);
